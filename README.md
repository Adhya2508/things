# Customer Segmentation & Personalization Agent — Retail Banking
...
| 6a–6d | Intent-specific filter nodes | Trim/round/prune the raw rows down to only what's needed (see **Token Optimization** below) |
...
The router (Node 2) uses a strict decision hierarchy, checked in this order:

1. **DIRECT_QUERY** — any ranking/list/top-N request ("top 10", "highest", "richest", "show users who…"). Always forces `ORDER BY` + `LIMIT`.
2. **EDA** — portfolio-wide aggregates only ("average", "total", "overall", "distribution"). Explicitly excludes anything that looks like a ranked list — a "top N" question is never classified as EDA even though it's technically an aggregate-adjacent question.
3. **EXPLAINABILITY** — explicit customer-ID audits ("why", "explain", "audit customer C10001").
4. **SEGMENTATION** — explicit ML/clustering language only ("segmentation model", "clustering", "k-means"). Always pulls the same 5 core behavioral columns (`CUST_ID`, `BALANCE`, `PURCHASES`, `CREDIT_LIMIT`, `PAYMENTS`) so downstream reasoning is never working off a single column by accident.
5. **UNSUPPORTED** — anything unrelated to the dataset, or any write-operation attempt (`delete`, `update`, etc.) disguised as a question. Bypasses the LLM summarizer entirely and returns a fixed, hardcoded string — no model call, no risk of the model improvising an inconsistent or overly specific refusal message.

A semantic synonym bridge in the same prompt maps colloquial phrasing ("money", "richest", "borrowed", "paid off") onto the exact schema columns, so the router doesn't need the user to know column names.
...
| **SEGMENTATION** | Arbitrary column selection risks single-dimension reasoning (the bug fixed above) | Capped to 10 rows, pruned to exactly the 5 core behavioral columns, floats rounded |
...
├── docker-compose.yml
├── .env.example
├── .env                  (create locally — see below, gitignored)
├── .gitignore
...
- Spins up an nginx container serving `index.html` on port 80, reverse-proxying `/webhook/*` straight to n8n internally — so the browser never needs CORS configuration at all
...
- **Efficient / cost-optimized** — intent-based filtering means every response pays only for the tokens its specific answer needs (see table above); `UNSUPPORTED` requests skip the summarizer LLM call entirely.
